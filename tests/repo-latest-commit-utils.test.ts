...
describe('mapGitHubCommitToCommitListItem', () => {
  test('maps a full GitHub commit list item without commitsUrl', () => {
    const payload = mapGitHubCommitToCommitListItem({
  ...
  test('returns null for empty or sha-less items', () => {
    expect(mapGitHubCommitToCommitListItem(null)).toBeNull();
    expect(mapGitHubCommitToCommitListItem(undefined)).toBeNull();
    expect(mapGitHubCommitToCommitListItem({ sha: '' })).toBeNull();
    expect(mapGitHubCommitToCommitListItem({ sha: '   ' })).toBeNull();
  });

  test('uses an empty message when commit message is missing', () => {
    expect(mapGitHubCommitToCommitListItem({ sha: '1234567' })).toEqual({
      sha: '1234567',
      shortSha: '1234567',
      message: '',
      committedAt: null,
