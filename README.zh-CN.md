...
仪表盘用户设置（包括字体选择、视图分组和自定义搜索标签页）通过名为 `userSettings` 的 Nitro storage mount 持久化。这与 GitHub token / session 存储相互独立。

默认情况下，GitPulse 使用本地文件系统路径 `./.data/user-settings`，该目录已被 git 忽略。生产部署，尤其是 serverless、容器或多实例部署，应将该 mount 配置为持久化存储。

| 环境变量                                     | 必填 | 默认值                  | 说明                                       |
| -------------------------------------------- | ---- | ----------------------- | ------------------------------------------ |
| `NUXT_GITPULSE_USER_SETTINGS_STORAGE_DRIVER` | 否   | `fs`                    | 用户设置使用的 Nitro/unstorage driver      |
| `NUXT_GITPULSE_USER_SETTINGS_STORAGE_BASE`   | 否   | `./.data/user-settings` | `fs` driver 或所选 driver 使用的 base 路径 |
