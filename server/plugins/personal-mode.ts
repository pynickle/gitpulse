...
const PERSONAL_MODE_STARTUP_FAILURE =
  'GitPulse personal mode failed to start: AUTH_PERSONAL_PAT could not be validated against GitHub. Confirm the token is current and allowed to access GitHub user identity.';
...
export default defineNitroPlugin(async (_nitroApp) => {
  ...
  if (!personalPat) {
    console.error('[auth] Personal mode startup validation failed: AUTH_PERSONAL_PAT is missing.');
